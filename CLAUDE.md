...
- `dev/release/build.py` builds the reproducible Composer artifact; `install-smoke.py` validates archive hashes and installation against an existing dependency tree. Neither publishes anything. Keep the overview README and focused guides under docs current; keep benchmark details in dev/bench.
